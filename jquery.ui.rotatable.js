...
(function ($) {
    $.fn.rotate = function (angle, options) {
        if (angle == null) {
            ...
            var centerX = options ? options.rotationCenterX : false;
            var centerY = options ? options.rotationCenterY : false;
            return this.data("angle", angle)
                .css('transform-origin', centerX + '% ' + centerY + '%')
                .css('-ms-transform-origin', centerX + '% ' + centerY + '%') /* IE 9 */
                .css('-webkit-transform-origin',
                  centerX + '% ' + centerY + '%')/* Chrome, Safari, Opera */
...
$.widget("ui.rotatable", $.ui.mouse, {
    ...
    performRotation: function (angle) {
        this.element.rotate(angle*180/Math.PI, this.options);
    },
